        ...
        if (viewModel.paymentIntent != null) {
            ...
            if (viewModel.state == SendCoinsViewModel.State.INPUT) {
                if (blockchainState != null && blockchainState.replaying) {
                    ...
                    final ExchangeRate exchangeRate = amountCalculatorLink.getExchangeRate();
                    final Fiat localAmount = exchangeRate.coinToFiat(dryrunTransaction.getFee());

                    final long vsize = dryrunTransaction.getFee().multiply(1000).divide(fees.get(feeCategory));
                    final Coin dynspvb = fees.get(feeCategory).divide(1000);
                    final Fiat fiatDynspvb = exchangeRate.coinToFiat(dynspvb);
                    final Coin calcspvb = dryrunTransaction.getFee().divide(vsize);

                    hintView.setText(getString(hintResId, dryrunTransaction.getFee().getValue(), btcFormat.format(dryrunTransaction.getFee()),
                        GenericUtils.currencySymbol(localAmount.getCurrencyCode()), Constants.LOCAL_FORMAT.format(localAmount)));
